...
alias gtat='git status | grep -A 50 '\''Untracked files:'\'' | awk '\''NR>3 {print last} {last=$0}'\'' | dmenu | xargs git add'
...
alias gtd='git diff'
alias gtdm='git status | tee /dev/tty | grep --color=always modified | awk '\''{print $2}'\'' | dmenu-mac | xargs git diff'
